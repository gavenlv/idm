...
CAST_OPS = {"Cast"}
AGG_OPS = {"Sum", "Avg", "Count", "Min", "Max", "Group"}
ARITH_OPS = {"Add", "Sub", "Mul", "Div", "Mod"}
FUNC_OPS = {"Upper", "Lower", "Trim", "Substring", "Concat", "Coalesce", "If"}
...
def _infer_transform_type(node: Any) -> tuple[str, str]:
    ...
    if cls in CAST_OPS:
        return "cast", expr
    if cls in {"RenameAlias"}:
        return "rename", expr
    if cls in DIRECT_OPS:
        return "direct", expr
    if cls in ARITH_OPS or cls in FUNC_OPS:
        return "expression", expr
    if cls in {"Case"}:
        return "derivation", expr
    if cls in {"Alias"}:
        return "passthrough", expr
    return "expression", expr
