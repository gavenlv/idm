...
class Schema(Base, UUIDMixin, TimestampMixin):
    ...
    database_id: Mapped["uuid.UUID"] = mapped_column(ForeignKey("databases.id", ondelete="CASCADE"), index=True, nullable=False)  # type: ignore[name-defined]  # noqa: F821
    name: Mapped[str] = mapped_column(String(128), nullable=False)
    description: Mapped[str | None] = mapped_column(String(1024), nullable=True)

    database: Mapped["Database"] = relationship(back_populates="schemas")
    tables: Mapped[list["TableAsset"]] = relationship(back_populates="schema", cascade="all, delete-orphan")
